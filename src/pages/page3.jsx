...
import {
    ...
    IconButton, Grid, Paper, Modal, Avatar, Tooltip, CircularProgress, Alert
} from '@mui/material';
import {
    Language, Report,
    ArrowBack, Eject, ArrowForward, Replay, Analytics
...
// Memoized Components
const MemoizedQuestionNumBox = React.memo(({ current, status, onClick, children }) => (
    <QuestionNumBox $current={current} status={status} onClick={onClick}>
        {children}
    </QuestionNumBox>
));

const QuestionPalette = React.memo(({ questions, currentQuestionIndex, questionStatus, navigateToQuestion }) => {
    return (
        <Box>
...
const UserProfile = React.memo(({ authState }) => (
    <Box sx={{
        display: 'flex', flexDirection: 'column', alignItems: 'center', pb: 2,
        borderBottom: `1px solid ${theme.palette.divider}`,
    }}>
        <Avatar src={authState?.user?.image} alt={authState?.user?.name}
...
const QuestionStatusLegend = React.memo(() => (
    <Box sx={{
        borderTop: `1px solid ${theme.palette.divider}`, 
        borderBottom: `1px solid ${theme.palette.divider}`,
        pt: 2, pb: 2, display: 'flex', flexWrap: 'wrap', justifyContent: 'space-around', gap: 1,
    }}>
        {[
            ...
            { label: 'Thinking', color: 'primary.main' },
        ].map((item, idx) => (
            <Box key={idx} sx={{
                display: 'flex', alignItems: 'center', fontSize: '13px',
                flexBasis: 'calc(50% - 8px)', maxWidth: 'calc(50% - 8px)', whiteSpace: 'nowrap',
            }}>
                <Box sx={{
                    width: 16, height: 16, borderRadius: '3px', mr: 1,
                    backgroundColor: item.color, border: `1px solid ${theme.palette.divider}`,
                }} />
                {item.label}
            </Box>
        ))}
...
const SidebarButtons = React.memo(() => (
    <Box sx={{
        mt: 'auto', borderTop: `1px solid ${theme.palette.divider}`,
        pt: 2, pb: 4, display: 'flex', flexDirection: 'column', gap: 1,
    }}>
        <Button variant="contained"  fullWidth>View Question Paper</Button>
...
const CompletionScreen = React.memo(({ paperId, navigate }) => (
    <Box sx={{
        display: 'flex', flexDirection: 'column', justifyContent: 'center',
        alignItems: 'center', height: '100vh', textAlign: 'center', p: 3
    }}>
        ...
        <Box sx={{ display: 'flex', gap: 2 }}>
            <Button variant="outlined" startIcon={<Replay />} onClick={() => window.location.reload()}>
                Retake Test
            </Button>
            <Button variant="contained" startIcon={<Analytics />} onClick={() => navigate('/page4')}>
                View Results
            </Button>
        </Box>
    </Box>
...
const Page3 = () => {
    ...
    const timerValue = useMemo(() => formatTime(timeLeft), [timeLeft, formatTime]);
    ...
    const debouncedSaveAnswer = useMemo(() => {
        ...
        return (question, optionIndex) => {
            ...
            timeout = setTimeout(async () => {
                ...
                const payload = {
                    data: question.qid,
                    response: optionIndex !== null ? String(optionIndex) : null
                };
                console.log('Saving answer to backend:', payload);
                try {
                    await api.post(`/testpaper/questions/${paperId}`, payload);
                } catch (err) {
                    const serverError = err.response?.data?.message || err.message;
                    setError(`Could not save answer: ${serverError}`);
                }
            }, 300);
        };
    }, [api, paperId]);
        ...
        setCurrentQuestionIndex(prevIndex => {
            ...
            if (prevQuestion) {
                ...
                setQuestionStatus(prev => {
                    ...
                    if (prev[prevQuestionId] === 'current' || prev[prevQuestionId] === 'not-answered') {
                        if (answers[prevQuestionId]) {
                            newStatus[prevQuestionId] = 'answered';
                        } else {
                            newStatus[prevQuestionId] = 'not-answered';
                        }
                    }
                    return newStatus;
                });
    ...
    const handleAnswerSelect = useCallback((question, optionIndex) => {
        ...
        debouncedSaveAnswer(question, optionIndex);
    }, [debouncedSaveAnswer]);
    ...
    const clearResponse = useCallback(() => {
        ...
        setQuestionStatus(prev => ({
            ...prev,
            [currentQuestion.id]: prev[currentQuestion.id] === 'answered-marked' 
                ? 'marked' 
                : 'not-answered'
        }));
        
        debouncedSaveAnswer(currentQuestion, null);
    }, [currentQuestion, debouncedSaveAnswer]);
    ...
    const markForReview = useCallback(() => {
        ...
        setQuestionStatus(prev => {
            ...
            if (currentStatus === 'answered' || currentStatus === 'answered-marked') {
                newStatus[questionId] = 'answered-marked';
            } else {
                newStatus[questionId] = 'marked';
            }
            return newStatus;
        });
    }, [currentQuestion]);
    ...
    }, [markForReview, currentQuestionIndex, questions.length, navigateToQuestion]);
    ...
    }, [currentQuestion, answers, currentQuestionIndex, questions.length, navigateToQuestion]);
    ...
    const confirmSubmit = useCallback(async () => {
        ...
        setLoading(true);
        const payload = { answers, timeSpent: timeLeft };
        console.log('Submitting test:', payload);
        try {
            await api.post(`/testpaper/submit/${paperId}`, payload);
            setIsTestCompleted(true);
            //destroying setAuthState(null); 
        } catch (e) {
        ...
        if (!isPaused && timeLeft > 0 && !loading && !isTestCompleted) {
            const timerInterval = setInterval(() => {
                setTimeLeft(prev => {
                    ...
                    } else if (newTime <= 1200 && newTime > 600 && timerColor !== 'warning.main') {
    ...
    useEffect(() => {
        if (timeLeft === 0 && !loading && questions.length > 0 && !isTestCompleted) {
            (async () => {
                await confirmSubmit();
                navigate(`/page4/`);
            })();
        }
    }, [timeLeft, loading, questions.length, isTestCompleted, confirmSubmit, navigate, paperId]);

    useEffect(() => {
        if (sections.length > 0) {
            const currentQuestionNumber = currentQuestionIndex + 1;
            const foundSection = sections.find(
                (section) => currentQuestionNumber >= section.start && currentQuestionNumber <= section.end
            );
            if (foundSection) {
    ...
    const handleSectionClick = useCallback((startQuestionNumber) => {
                ...
                <StyledContainer>
                            ...
                            <ScrollableContent>
                                {!loading && !error && questions.length > 0 ? (
                                    ...
                                    <Box sx={{
                                        ...
                                        height: '100%', flexDirection: 'column', textAlign: 'center',
                    ...
                    <BottomNav elevation={3}>
                        <Box sx={{
                            ...
                            <Box sx={{ position: 'relative', display: { xs: 'block', md: 'none' } }} ref={popupRef}>
                    ...
                    >
                        <Box sx={{
                            ...
                            <Typography id="submit-modal-description" sx={{ mt: 2 }}>
                                Are you sure you want to submit the test? You can also retry if you have time left.
                            </Typography>
